 ...
 * @author BAP1
 */
public final class PanelScatterBlob1 extends javax.swing.JPanel {
    ...
    private float[] distinct_color_indexes;
    private long rand_seed;
    private String[] items;
    private String title="Basic Scatter Plot";

    public PanelScatterBlob1() {
        initialize();
        repaint();
    }

    public PanelScatterBlob1(CMatrix cm) {
        this.cm = cm;
        initialize();
        float[][] dm=cm.toFloatArray2D();
        float[] d=FactoryUtils.getDistinctValues(dm[dm.length-1]);
    ...
    public PanelScatterBlob1(CMatrix cm, TFigureAttribute attr) {
        this.cm = cm;
    ...
    public void paint(Graphics gr1) {
        ...
        drawYAxis(gr, px, py, r.width, cm.toFloatArray2D());
        drawXAxis(gr, px, py, r.height, w, cm.toFloatArray2D());
        BlobPoint[] mp = mappingDataToScreenCoordinates(cm.toFloatArray2D(), w, h, px, py);
    ...
    private void drawYAxis(Graphics2D gr, int x0, int y0, int w, float[][] d) {
        float maxY = FactoryUtils.getMaximum(d[1]);
        float minY = FactoryUtils.getMinimum(d[1]);
        ...
        for (int i = 1; i < n; i++) {
            ...
            gr.drawString(FactoryUtils.formatFloatAsString(yVal[i], 1) + "", x0 - 70, (int) (y0 - i * dY + shift) + art);
            q = (int) (y0 - i * dY + shift);
            gr.setColor(Color.white);
    ...
    private void drawXAxis(Graphics2D gr, int x0, int y0, int h, int w, float[][] d) {
        float maxX = FactoryUtils.getMaximum(d[0]);
        float minX = FactoryUtils.getMinimum(d[0]);
        ...
        for (int i = 1; i < n; i++) {
            ...
            gr.drawString(FactoryUtils.formatFloatAsString(xVal[i], 1) + "", q - 10, y0 + 30);
            gr.setColor(Color.white);
            gr.drawLine(q, y0 + 2, q, y0 - h);
        }
    }

    private BlobPoint[] mappingDataToScreenCoordinates(float[][] d, int w, int h, int px, int py) {
        ...
        float deltaX = Math.abs(maxX - minX);
        int m = 160;
        float cellWidth = (w - (px + m)) / deltaX;
        float cellHeight = ((h - 140) / deltaY);
        int last_index=d.length-1;
        distinct_color_indexes=FactoryUtils.getDistinctValues(d[last_index]);
        distinct_colors=FactoryUtils.getRandomColors(distinct_color_indexes.length,rand_seed);
