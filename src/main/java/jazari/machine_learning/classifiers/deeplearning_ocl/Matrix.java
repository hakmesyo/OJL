...
public class Matrix implements Serializable {
    ...
    public Matrix copy() {
        return new Matrix(data.toFloatArray2D());
    }
    ...
    public void fill(float value) {
        data = CMatrix.getInstance(FactoryMatrix.matrixFloatValue(numRows(), numColumns(), value));
    }
    ...
    public Matrix add(float n, Matrix m2) {
        checkSize(m2);
        float[][] d = getData();
        float[][] d2 = m2.getData();
        int r = d.length;
