 ...
 */
package jazari.machine_learning.evaluater;
...
public class FactoryEvaluation {
    ...
    public static float[] simulated = new float[0];
    public static float[] observed = new float[0];
...
//            float[] simulated = new float[0];
//            float[] observed = new float[0];
//            float[] sim = new float[0];
//            float[] obs = new float[0];
...
//                if (show_plot) {
//                    float[][] d = new float[2][sim.length];
//                    d[0] = obs;
//                    d[1] = sim;
...
//                }
                simulated = FactoryUtils.concatenate(simulated, FactoryUtils.toFloatArray1D(eval.evaluateModel(clsCopy, validation)));
                observed = FactoryUtils.concatenate(observed, FactoryUtils.toFloatArray1D(validation.attributeToDoubleArray(validation.classIndex())));
...
//                observed = FactoryUtils.mean(observed,validation.attributeToDoubleArray(validation.classIndex()));
            ...
            if (show_plot) {
                float[][] d = new float[2][simulated.length];
                d[0] = observed;
                d[1] = simulated;
                CMatrix f1 = CMatrix.getInstance(d);
...
//                System.out.println("Classifier: " + model.getClass().getName() + " " + Utils.joinOptions(model.getCapabilities()));
                System.out.println("Dataset: " + randData.relationName());
                System.out.println("Folds: " + folds);
                System.out.println("Seed: " + 1);
                System.out.println();
...
//                System.out.println("Classifier: " + model.getClass().getName() + " " + Utils.joinOptions(model.getOptions()));
    ...
    private static Evaluation doTest(boolean isTrained, Classifier model, Instances train, Instances test, boolean show_text, boolean show_plot, TFigureAttribute attr) {
        ...
        Evaluation eval = null;
        try {
//            float[] simulated = null;
            eval = new Evaluation(train);
            if (isTrained) {
                simulated = FactoryUtils.toFloatArray1D(eval.evaluateModel(model, test));
            } else {
                Classifier clsCopy = AbstractClassifier.makeCopy(model);
                clsCopy.buildClassifier(train);
                System.out.println("\n benchmarking  ******************************************");
                t1=FactoryUtils.toc("training:",t1);
                simulated = FactoryUtils.toFloatArray1D(eval.evaluateModel(clsCopy, test));
                t1=FactoryUtils.toc("testing:",t1);
                System.out.println("************************************************************");
            }
            if (show_plot) {
                observed = FactoryUtils.toFloatArray1D(test.attributeToDoubleArray(test.classIndex()));
                float[][] d = new float[2][simulated.length];
                d[0] = observed;
                d[1] = simulated;
                CMatrix f1 = CMatrix.getInstance(d);
