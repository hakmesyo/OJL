 ...
 */
package jazari.deep_learning.ai.djl.examples.denemeler.mnist;
...
//        ImageFolder dataset = initDataset("c:/ai/djl/mnist_test");
        ...
        float lr=03.f;
        Optimizer opt=Optimizer.adam().optLearningRateTracker(Tracker.fixed(lr)).build();
        TrainingConfig config = setupTrainingConfig(loss,opt);

        try (Model model = Models.getModel(Models.NUM_OF_OUTPUT, Models.IMAGE_WIDTH, Models.IMAGE_HEIGHT); // empty model instance to hold patterns
    ...
    private static TrainingConfig setupTrainingConfig(Loss loss,Optimizer opt) {
        return new DefaultTrainingConfig(loss)
                .optOptimizer(opt) // Optimizer
