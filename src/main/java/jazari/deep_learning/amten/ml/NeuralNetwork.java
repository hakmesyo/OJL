...
import jazari.deep_learning.amten.ml.matrix.MatrixElement;
...
//    }
    private Matrix createTheta(int rows, int cols) {
        Matrix theta = MatrixUtils.random(rows, cols, 123);
    ...
    private Matrix[] getGradients(Matrix x, Matrix y, double weightPenalty, Matrix[] dropoutMasks, int batchSize) {
        ...
        if (weightPenalty > 0) {
            ...
            for (int thetaNr = 1; thetaNr < numLayers; thetaNr++) {
                ...
                //System.out.println("thetaNr:"+thetaNr+" grad:"+grad.numRows()+","+grad.numColumns()+" theta:"+theta.numRows()+","+theta.numColumns());
        ...
        for (int batchNr = 0; batchNr < batchesX.size(); batchNr++) {
            ...
            Runnable batchGradientCalculator = new Runnable() {
                public void run() {
                    ...
                    Matrix[] dropoutMasks = useDropout ? generateDropoutMasks(by.numRows(),321) : null;
                    myThetasLock.readLock().lock();
